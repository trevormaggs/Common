...
public final class CommandLineReader
{
     ...
     */
    public CommandLineReader(String[] args, boolean dbg)
    {
    ...
    // Logic Improvement: Move away from string-suffix checks
    private static List<String> robustSanitise(String[] args)
    {
        ...
        for (int i = 0; i < args.length; i++)
        {
            ...
            // If a token is just "=" or ",", it belongs to the previous and next tokens
            if ((current.equals("=") || current.equals(",")) && !tokens.isEmpty() && i + 1 < args.length)
            {
                String prev = tokens.remove(tokens.size() - 1);

                tokens.add(prev + current + args[++i]);
            }
    ...
    private static boolean robustIsNegativeNumber(String token)
    {
        return token.matches("^-?\\d+(\\.\\d+)?$") && Double.parseDouble(token) < 0;
    }
    ...
    private void robustHandleLongOption(String token, PeekingIterator<String> iter) throws ParseException
    {
        String key;
        ...
        if (token.contains("="))
        {
            ...
            key = stripLeadingDashes(parts[0]);
            attachedValue = parts[1];
        }

        else
        {
            key = stripLeadingDashes(token);
        }

        FlagOptionRule rule = flagRuleMap.get(key);

        if (rule == null)
        {
            throw new ParseException("Unknown long option: [" + token + "]", 0);
        }
        ...
        if (rule.expectsArgument())
        {
            ...
            if (attachedValue != null)
            {
                ...
                if (attachedValue.contains(","))
                {
                    processRangeValues(attachedValue);
                }

                else
                {
                    processSingleValue(attachedValue);
                }

                activeFlagView.setOptionDone();
            }

            // Case 2: --option value (consume next token)
            else if (iter.hasNext() && isValidArgument(iter.peek()))
            {
                processSingleValue(iter.next());
                activeFlagView.setOptionDone();
            }
            ...
            {
                throw new ParseException("Option [" + rule.getOptionName() + "] requires an argument.", 0);
            }
        }
    ...
    private void robustHandleShortOption(String token, PeekingIterator<String> iter) throws ParseException
    {
        // Skip the leading '-'
        for (int i = 1; i < token.length(); i++)
        {
            ...
            if (rule == null)
            {
                // Handle unknown character in a cluster
                processRemainingArgument("-" + key);
                continue;
            }
            ...
            if (rule.expectsArgument())
            {
                String remaining = token.substring(i + 1);

                // Case 1: Value is attached to the flag (-fValue or -f=Value)
                if (!remaining.isEmpty())
                {
                    ...
                    {
                        processRangeValues(remaining);
                    }
                    else
                    {
                        processSingleValue(remaining);
                    }
                    activeFlagView.setOptionDone();
                    return; // Entire token is now consumed
                }

                // Case 2: Value is the next token (-f Value)
                else if (iter.hasNext() && isValidArgument(iter.peek()))
                {
                    String nextToken = iter.next();
                    processSingleValue(nextToken);
                    activeFlagView.setOptionDone();
                    return;
                }

                // Case 3: Missing mandatory argument
                else if (rule.isRequired())
                {
                    throw new ParseException("Option [" + rule.getOptionName() + "] is missing an argument.", 0);
                }
            }
            else
            {
                // Standalone flag (-v), mark as done and move to next character
                activeFlagView.setOptionDone();
            }
        }
    }
    ...
    private void robustHandleExtendedShortOption(String token, PeekingIterator<String> iter) throws ParseException
    {
        String optName;
        String attachedValue = null;

        // Handle the separator if present (e.g., -value=53)
        if (token.contains("="))
        {
            String[] parts = token.split("=", 2);
            optName = parts[0]; // Keep the dash for matching rule
            attachedValue = parts[1];
        }
        ...
        // Look up the rule using the flag name (including the leading dash)
        FlagOptionRule rule = null;

        for (FlagOptionRule r : flagRuleMap.values())
        {
            if (r.isExtendedShortOption() && optName.equals(r.getOptionName()))
            {
                rule = r;
                break;
            }
        ...
        if (rule.expectsArgument())
        {
            // Case 1: Value attached via separator (-E=V)
            if (attachedValue != null)
            {
                activeFlagView.setSeparator();
                if (attachedValue.contains(","))
            ...
            // Case 2: Value concatenated directly (-EV) - identified by token length
            else if (token.length() > rule.getOptionName().length())
            {
                String val = token.substring(rule.getOptionName().length());
                processSingleValue(val);
                activeFlagView.setOptionDone();
            }

            // Case 3: Value is the next token (-E V)
            else if (iter.hasNext() && isValidArgument(iter.peek()))
            {
                processSingleValue(iter.next());
                activeFlagView.setOptionDone();
            ...
            // Case 4: Missing mandatory argument
            else if (rule.isRequired())
            {
                throw new ParseException("Extended option [" + rule.getOptionName() + "] is missing an argument.", 0);
            }
        }

        else
        {
            // Standalone extended flag
            activeFlagView.setOptionDone();
        }
    }
    ...
    {
        if (k < 0 || k >= standaloneTokens.size())
        {
            return "";
        }

        return standaloneTokens.get(k);
    }
    ...
    public void robustParse() throws ParseException
    {
        if (flagRuleMap.isEmpty())
        {
            throw new ParseException("Rules for command line options must be defined prior to parsing.", 0);
        }

        PeekingIterator<String> iter = new PeekingIterator<>(arguments);
        ...
        while (iter.hasNext())
        {
            ...
            {
                robustHandleLongOption(token, iter);
            }
            else if (isExtendedShortOption(token))
            {
                robustHandleExtendedShortOption(token, iter);
            }
            else if (isShortOption(token))
            {
                robustHandleShortOption(token, iter);
            }
            else
            {
                processRemainingArgument(token);
